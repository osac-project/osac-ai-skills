...
- `useCaseCards[].items[].status` uses the same real-Jira-status mapping as `featureInventory`/`customerCoverage` (`"Done ✅"`, `"In Progress"`, `"In Review"`, `"Planned"`) for every item, target and prior alike. Consumers should only surface it as a caveat for prior-version items (`isTarget: false`) whose status is not one of the two finished values defined below — `"Done ✅"` or `"Closed ✅"` — a target-version item being Planned/In Progress is expected in a forward-looking report and needs no caveat.
