...
- **Flag unfinished prior-version work.** Prior-version items (not the target) are fetched regardless of Jira status (see N-1/N-2 rules above), so a prior version can include a feature that never actually shipped. For any prior-version item whose mapped status is not one of the two finished values — `"Done ✅"` or `"Closed ✅"` (the same canonical mapping used everywhere in this report; see the JSON schema's `status` rule) — append its status in parentheses, e.g. "Feature Name (In Progress)", so it isn't mistaken for delivered work. Target-version items never need this — a forward-looking report showing Planned/In Progress work for the target version is expected.
