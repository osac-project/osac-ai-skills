...
Unlike a consumer workspace (e.g. `osac-workspace`, where `create-pr`'s
Step 1 resolves a `$WORKSPACE_ROOT` separate from wherever `skills/`
happens to live), **this repo is `skills/`'s canonical home** — there is no
self-check-then-fallback needed here. `$OSAC_AI_SKILLS_DIR` is resolved
once, in Step 1's remote resolution (see
[resolve-remotes.md](resolve-remotes.md)), and every path in this
document — the config file itself and every `skill:` value inside it — is
read from `${OSAC_AI_SKILLS_DIR}/<path>` directly.

**Reviewer git-command anchoring is separate from file-path resolution.**
`$OSAC_AI_SKILLS_DIR` only governs where `create-pr` itself reads
config/skill *files* from. A spawned reviewer's own git commands
(merge-base, diff) run wherever the reviewer starts — anchor them
explicitly by substituting `{repo_dir}` (`$REPO_DIR`, the consumer repo
being submitted, *not* this repo) into the prompt and instructing the
reviewer to run its git commands against that directory (see the shipped
`prompt_template`).
...
| 12 | Every entry in the enabled set has a `skill` value that's sandboxed (under `skills/`, no `..`, not absolute, ends in `SKILL.md`), contains no embedded newline or control character, and — resolving any symlink in the path, not just checking the lexical string — its real path exists and stays under `${OSAC_AI_SKILLS_DIR}/skills/`. Not checked for disabled entries. | the entry and path |
...
string as closely as they would code. `{repo_dir}` needs no equivalent
...
Check 12's real-path requirement is defense in depth: a `skill:` value that
lexically satisfies "under `skills/`, ends in `SKILL.md`" could still
resolve, via a symlink added under `skills/` in the same PR as a crafted
`skill:` value, to a file entirely outside `${OSAC_AI_SKILLS_DIR}/skills/`
— and that file's contents then become literal instructions handed to a
tool-using background agent. This repo's own `skills/` directory has no
such symlinks today (unlike a consumer workspace that fans this repo's
skills out alongside a separately-vendored `flightctl/ai-workflows`), so
this isn't guarding against an existing pattern here specifically — it's a
cheap, always-correct check against a symlink a future PR could introduce,
whether accidentally or adversarially.
...
place*, not deletion (see Mandatory Reviewers below).
...
a clean result is a solo `NONE` row.
...
3. **Tolerate leading prose before the table, but only if it doesn't itself
   ...
   - If the discarded prefix contains neither kind of claim, drop it —
     there is no length limit on prose that passes this check: this is a
     content check, not a length check, and closes a gap length never
     could — a preamble can be short and still narrate a finding. An
     occurrence of the header text that isn't immediately followed by a
     genuinely complete, valid table doesn't count as that point; if no
     such point exists anywhere in the response, no tolerance applies and
     normalization stops here. Trailing content after the table is
     **never** tolerated — the table must be the exact, unbroken end of
     the response.
...
**Why the leading-paragraph tolerance exists:** most genuinely-clean
reviews come back as an explanatory sentence before the payload, even
under an explicit no-preamble instruction — a strict no-preamble rule is a
real, frequent source of false `INVALID`s, not a theoretical edge case.
The exact-match header-row anchor (as opposed to, say, a JSON array's bare
`[`) matters because it makes false positives effectively impossible — a
preamble sentence would have to coincidentally contain the literal
4-column header string, *and* be immediately followed by a complete,
well-formed table with nothing after it, to be mistaken for the payload
boundary. A length-based cap on the tolerated preamble was considered and
rejected: it would reject legitimate, substantive explanations that
happen to run long while doing nothing to stop a short preamble that
narrates a real finding and then emits a fabricated clean `NONE` row —
the actual failure mode this contract needs to close. The content-based
check above (does the discarded prefix name a finding the table doesn't
report) closes that hole directly without introducing a length-based
false-positive class: a discarded preamble that names a concrete finding
the table then fails to report blocks the discard regardless of how short
the preamble is, while a preamble that names a finding the table *does*
correctly report, or that merely reasons about severity in the abstract,
still discards cleanly no matter how long it is.
...
"mostly valid" one), any trailing content after the table, a response
where the header text appears but is never followed by a genuinely
complete table, or a stray `verdict: pass`/`verdict: blocked` line
(case-insensitive) anywhere in the raw response. Any spawned reviewer's
...
one of these things anyway: this repo's `OWNERS` file governs approval
(Prow-style approvers/reviewers lists, not GitHub-native CODEOWNERS —
there's no per-path ownership mechanism here to route this specific file
to a particular reviewer), and human approval judgment is the actual
control against a deliberate, reviewed bypass, same as for any other
change to this repo.
...
1. Write the reviewer's own `SKILL.md`, following `skills/review-gate/
   ...
   INVALID` string. `ponytail-review`'s `## Output` section shows the
   pattern for stating this explicitly, including how to suppress a
   native section beyond `## Output` itself (its `## Scoring` section).
