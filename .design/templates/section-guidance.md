...
- Describe Proto schema shape using the standard object shape (`id`, `Metadata`, `<Type>Spec`, `<Type>Status`) and conventions in [`osac/fulfillment-service/docs/API.md`](https://github.com/osac-project/osac/blob/main/fulfillment-service/docs/API.md). Use prose or lists for field names, types, defaults, and constraints. Do not include literal `.proto` source syntax.
